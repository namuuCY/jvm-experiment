...
![알고리즘 정합성 테스트](/documents/images/step1_image1.png)
...
![프로파일러](/documents/images/step1_image2.png)
...
- 다만, 아무런 옵션 없이 프로파일러를 실행할 경우 GC와 관련된 기록을 확인하기가 힘듭니다. 실험을 위해 자세한 프로파일링이 필요합니다.
  - 따라서 아래의 VM 옵션을 추가합니다.
```
-XX:StartFlightRecording=dumponexit=true,filename=my_recording.jfr,settings=profile
```
- 각각의 옵션은 아래와 같습니다. [🔗 출처](https://docs.oracle.com/en/java/javase/21/jfapi/flight-recorder-configurations.html) 
  - `-XX:StartFlightRecording=dumponexit=true` : JVM이 켜지자마자 JFR 엔진을 가동, 어플리케이션이 종료될때 메모리에 있던 기록을 dump로 반환
  - `filename=my_recording.jfr` : 저장할 파일의 이름과 위치
  - `settings=profile` : 정밀한 프로파일링
...
![콘솔 출력](/documents/images/step1_image5.png)
- flame graph, Call tree를 통해 병목 지점 시각화
  - solveNQueens 메서드가 전체 어플리케이션 시간의 98.9% 를 차지하고 있어 병목임을 알 수 있습니다.
![flame graph](/documents/images/step1_image3.png)
![call tree](/documents/images/step1_image4.png)
- Timeline을 통해 리소스(CPU)패턴을 알 수 있습니다.
  ...
  - (⚠️ update) 다만, 이후 정밀한 실험을 위해서 Step 2 부터는 DevTools의 의존성을 제거하고 실험합니다.
![timeline](/documents/images/step1_image6.png)
